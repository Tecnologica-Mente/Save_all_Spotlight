...
REM * e anche quelle di Bing Wallpaper App                                                         *
REM *                                                                                              *
...
for /r "%origine%" %%f in (*) do (
   ...
   if errorlevel 1 (
      ...
      set /a count1+=1
      echo Copiato: %%~nxf  [!count1!]
   )
)

:fase2_end
echo.
echo %count1% immagini totali copiate
echo FASE 2 COMPLETATA
echo.

echo FASE 3. SALVATAGGIO DEI FILE DI BING WALLPAPER APP
REM Imposta le cartelle
set "origine=%LOCALAPPDATA%\Packages\Microsoft.BingWallpaper_8wekyb3d8bbwe\LocalState\images\Bing"
set "destinazione=%~dp0%folder_name%"
set count2=0
...
REM Copia SOLO i file JPG con verifica (solo se la cartella "origine" esiste)
for /r "%origine%" %%f in (*.jpg) do (
   copy "%%f" "%destinazione%" >nul
   if errorlevel 1 (
      echo ERRORE copia %%f
   ) else (
      set /a count2+=1
      echo Copiato: %%~nxf  [!count2!]
   )
)

:fase3_end
echo.
echo %count2% immagini totali copiate
echo FASE 3 COMPLETATA
echo.

set /a totale=img_valide + count1 + count2
echo TOTALE IMMAGINI SALVATE (FASE 1 + FASE 2 + FASE 3): %totale%
echo *****************************
