...
REM * and also those of Bing Wallpaper App                                 *
REM *                                                                      *
...
for /r "%origine%" %%f in (*) do (
   ...
   if errorlevel 1 (
      ...
      set /a count1+=1
      echo Copied: %%~nxf  [!count1!]
   )
)

:fase2_end
echo.
echo %count1% total images copied
echo STEP 2 COMPLETED
echo.

echo STEP 3. SAVING BING WALLPAPER APP FILES
REM Set up folders
...
REM Copy ONLY verified JPG files (only if the "source" folder exists)
for /r "%origine%" %%f in (*.jpg) do (
   ...
   if errorlevel 1 (
      ...
      set /a count2+=1
      echo Copied: %%~nxf  [!count2!]
   )
)

:fase3_end
echo.
echo %count2% total images copied
echo STEP 3 COMPLETED
echo.

set /a totale=img_valide + count1 + count2
echo TOTAL IMAGES SAVED (STEP 1 + STEP 2 + STEP 3): %totale%
echo ****************************
